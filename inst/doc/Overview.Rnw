...
\date{25 July, 2008}
% FIXME: according to SVN log, your last change was 30 Oct
%   may use \today instead?
...
class is defined in \Rpackage{IRanges}. 
% FIXME shouldn't this be 'Biostrings'?
It represents DNA sequence
...
%% 
percent of reads were not aligned (level
\Rcode{NA}).
...
e.g., \Rclass{DNAStringSet}. 
% FIXME it is not easy to understand what you mean by 'underlying' here
Applying names to \Robject{colClasses} is
not required, but makes subsequent manipulation easier. We are now ready
to read our file
<<readXStringColumns>>=
...
each read and a (short!) % FIXME not clear what the 'short!' means here - 
% does it not work for long reference sequences (and how long?), or does it just become inefficient
reference sequence. To find reads close to
the most common read in the example above, one might say
<<srdistance>>=
...
\subsection{The \Rfunction{pileup} function}

The \Rfunction{pileup} function provides a convenient way to summarize where reads
align on reference sequences. \Rfunction{pileup} uses alignments
obtained from, e.\,g., \Rfunction{readAligned} or the
...
@ 
% FIXME wouldn't it be more efficient to first do 'colMeans' and then 
% set the (col)names?
%% 
