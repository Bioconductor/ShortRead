...
columns of maq mapview. */
{
   ...
   int i, actnreads, j;
   maqmap1_t read;
   static const char *eltnames[] = { "chromosome", "position", "strand", "readLength", 
      "alignQuality", "nMismatchBestHit", "nMismatchBestHit24", "mismatchQuality", 
      "nExactMatch24", "nOneMismatch24", "readId", "readSequence", "fastqScores" };
   ...
   if( i != MAQMAP_FORMAT_NEW ) {
      gzclose( mapfile );
      snprintf( buf, lbuf, "File '%s' is not a MAQ map file", 
   ...
   PROTECT( length = allocVector( INTSXP, actnreads ) );
   ...
   for( i = 0; i < actnreads; i++ ) {
	 ...
	 fastqbuf[j] = ( read.seq[j] & 0x3f ) + 33;   
      ...
      /* Copy the data */
      if( read.pos & 0x01 ) /* '-' strand? */
         INTEGER(start)[i] = ( read.pos >> 1 ) + read.size;
      else
         INTEGER(start)[i] = read.pos >> 1;
      INTEGER(seq   )[i] = read.seqid + 1;	
      INTEGER(dir   )[i] = ( read.pos & 0x01 ) + 1;
      INTEGER(length)[i] = read.size;
   ...
   PROTECT( df = allocVector( VECSXP, 13 ) );
   SET_VECTOR_ELT( df, 0, seq );
   SET_VECTOR_ELT( df, 1, start );
   SET_VECTOR_ELT( df, 2, dir );    
   SET_VECTOR_ELT( df, 3, length ); 
   SET_VECTOR_ELT( df, 4, aq );
   SET_VECTOR_ELT( df, 5, mm );    
   SET_VECTOR_ELT( df, 6, mm24 );  
   SET_VECTOR_ELT( df, 7, errsum );
   SET_VECTOR_ELT( df, 8, nhits0 );
   SET_VECTOR_ELT( df, 9, nhits1 );
   SET_VECTOR_ELT( df, 10, new_XStringSet_from_RoSeqs( "BString",   new_RoSeqs_from_BBuf( readid ) ) );
   SET_VECTOR_ELT( df, 11, new_XStringSet_from_RoSeqs( "DNAString", new_RoSeqs_from_BBuf( readseq ) ) );
   SET_VECTOR_ELT( df, 12, new_XStringSet_from_RoSeqs( "BString",   new_RoSeqs_from_BBuf( fastq ) ) );
